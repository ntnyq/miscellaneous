...
window.Modernizr = (function(i, e, u) {
      ...
      for (var b = 0, c, k = a.length; b < k; b++) {
        ...
        if ((c = h.type !== 'text')) {
          ...
          if (/^range$/.test(h.type) && h.style.WebkitAppearance !== u) {
            ...
            c =
              c.getComputedStyle &&
              c.getComputedStyle(h, null).WebkitAppearance !== 'textfield' &&
              h.offsetHeight !== 0
            l.removeChild(h)
          } else
            /^(search|tel)$/.test(h.type) ||
              (c = /^(url|email)$/.test(h.type)
                ? h.checkValidity && h.checkValidity() === false
                : h.value != M)
        }
        N[a[b]] = !!c
      }
  ...
  d.video = function() {
    ...
    if (b) {
      ...
      b.h264 =
        a.canPlayType('video/mp4; codecs="avc1.42E01E"') ||
